...
/**
 * Dual-threshold bite detector with duration tracking.
 *
 * A bite STARTS when BOTH are true simultaneously:
 *   1. Wrist upward velocity >= VELOCITY_THRESHOLD
 *   2. Wrist Y position      <= WRIST_HEIGHT_THRESHOLD (high in frame = near face)
 *
 * A bite ENDS when either condition drops back below its threshold.
 * Duration = time from first frame both were met to when either dropped.
 * Cooldown after each bite prevents double-counting the return swing.
 *
...
function detectBite(velocity, normY) {
  ...
  if (state.bitePhase === 'idle') {
    if (bothAbove) {
      // Both conditions just became true — start timing the bite
    ...
    if (!bothAbove) {
      // One or both conditions dropped — bite arc is over, record duration
    ...
    // else: still in bite — keep timing
  }
}
